...
const dragAndDrop = async () => {
  const query = `
    SELECT courseID, courseName, courseSubjectID, courseTerm, courseStartTime, courseEndTime, courseDaysOfWeek, prerequisites, corequisites
    FROM courses;
  `;
...
export const getFacultyProfile = async (userId) => {
  try {
    const [results] = await pool.query('SELECT * FROM faculty WHERE userID = ?', [userId]);
    return results.length ? results[0] : null;
  } catch (error) {
    throw error;
  }
